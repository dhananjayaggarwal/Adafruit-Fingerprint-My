...
/************************** DATA PACKETS CUSTOM STARTS**********************
#define GET_DATA_PACKET(...) \
  uint8_t data[] = {__VA_ARGS__}; \
...
#define SEND_DATA_PACKET(...) GET_DATA_PACKET(__VA_ARGS__); return packet.data[0];

*************************** DATA PACKETS CUSTOM ENDS**********************



***********************CUSTOM ACK PACKET***************************
#define GET_ACK_PACKET(...) \
  uint8_t data[] = {__VA_ARGS__}; \
  Adafruit_Fingerprint_Packet packet(FINGERPRINT_ACKPACKET, sizeof(data), data); \
  writeStructuredPacket(packet); \
  if (getStructuredPacket(&packet) != FINGERPRINT_OK) return FINGERPRINT_PACKETRECIEVEERR; \
  if (packet.type != FINGERPRINT_ACKPACKET) return FINGERPRINT_PACKETRECIEVEERR;

#define SEND_ACK_PACKET(...) GET_ACK_PACKET(__VA_ARGS__); return packet.data[0];
***********************CUSTOM ACK PACKET***************************/
...
/**************************************************************************/
/*!
    @brief  Send command to sensor about downloading character from upper computer to sensor
    @param bufferNumber to which template has to be stored 01h for buffer 1 and 02h for buffer 2
 

*/
/**************************************************************************/
uint8_t Adafruit_Fingerprint::downModel(uint8_t bufferNumber) {
  SEND_CMD_PACKET(FINGERPRINT_DOWNCHAR, bufferNumber);
}


/**************************************************************************/
/*!
    @brief  After receiving success of downModel above, start sending data to sensor buffer which was mentioned previously, 
            note that template size is 512 bytes and we sends 128bytes at a time so 3 data packets and last endof data packet is used
    @param type = 1 for DATA_PACKEt and type = 2 for ENDDATAPACKET
    @param lenghthOfData specifies total data length
    @param dataPtr is pointer to array which contains actual data
 

*/
/**************************************************************************/
void Adafruit_Fingerprint::sendData(uint8_t type, uint8_t lengthOfData, uint8_t * dataPtr){
  uint8_t data[128] = {0};
